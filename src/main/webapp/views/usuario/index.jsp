...
<body>
	...
	<main class="container">
		...
		<c:if test="${user.isAdmin()}">
			<div class="mb-3">
				<a href="/turismo-webapp/usuario/create.do" class="btn btn-primary"
			...
			<tbody>
				<c:forEach items="${usuarios}" var="tmp_user">
					<tr>
						...
						<td><c:out value="${tmp_user.disponibilidad}"></c:out></td>
						<td><c:out value="${tmp_user.preferencia }"></c:out></td>
						<td>
							<c:choose>
						...
						<td><c:if test="${user.admin && (!tmp_user.admin || tmp_user.id == user.id)}">
								<a href="/turismo-webapp/usuario/edit.do?id=${tmp_user.id}"
									class="btn btn-light rounded-0" role="button"><i
									class="bi bi-pencil-fill"></i></a>
								<a href="/turismo/usuario/delete.do?id=${tmp_user.id}"
